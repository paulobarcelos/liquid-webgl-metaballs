...
#define REFRACTION_RATIO 0.0
#define REFLECTION_DEPTH 0
#define REFLECTION_AMOUNT 0.2
...
const float TEXTURE_ASPECT = 640.0 / 480.0;
...
vec3 cube(in vec3 v) {
   ...
   float scale = (float(CUBEMAP_SIZE) - 1.) / float(CUBEMAP_SIZE) *0.5;
   if (abs(v.x) != M) v.x *= scale;
   if (abs(v.y) != M) v.y *= scale;
   if (abs(v.z) != M) v.z *= scale;
   return textureCube(cubemap, v).xyz;
}
...
void main ( void ){
	...
	float texScale = aspect/TEXTURE_ASPECT;	
	vec3 color = texture2D(texture,  ((gl_FragCoord.xy/texScale) / resolution.xy) * vec2(texScale,1.0) ).rgb;
	if(info.intersected){
	...
	color -=  pow(uvLength, 2.0) * 0.7; //vignette
